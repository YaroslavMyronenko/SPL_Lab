...
bot = telebot.TeleBot('7793189317:AAEmmz0eTkucJbnKrJKwgWbDYh2QG1Qc7ng') # токен бота

# // start - команда, яка запускає бота
@bot.message_handler(commands = ['start']) #реакція чат бота 
                                         #на стартову команду
def start_message(message):
    bot.send_message(message.chat.id, f'Привіт {message.from_user.first_name} я чат бот на базі Python!') #привітання користувача
    bot.send_message(message.chat.id, 'Введіть /help для отримання списку команд') #підказка користувачу
...
@bot.message_handler(content_types=['photo', 'video']) #реакція чат бота
def like_photo(message):
    bot.send_message(message.chat.id, 'Красунчик 😎') #відповідь на фото
...
def help_message(message):
    bot.send_message(message.chat.id, '🆘 Список команд:\n'
        ...
        'надіславши фото або відео, ви отримаєте відповідь "Красунчик 😎"\n'
