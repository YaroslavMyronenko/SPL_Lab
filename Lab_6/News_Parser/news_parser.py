...
def save_to_db(word_count, tag_count, links, images): 
    ...
    word_count_gt3 = {word: count for word, count in word_count.items() if len(word) > 3} # Фільтрація слів з довжиною більше 3 символів
    top_words = sorted(word_count_gt3.items(), key=lambda x: x[1], reverse=True)[:10] # Отримання топ-10 слів за частотою
