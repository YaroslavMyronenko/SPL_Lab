...
def remove_student():
    """Функція для видалення студента з файлу за номером
    з попереднім відображенням списку студентів"""
    ...
    try:
        # Читаємо дані прямо тут, без виклику read_file()
        with open(selected_path, 'r', encoding='utf-8', newline='') as csvfile:
            reader = csv.reader(csvfile)
            rows = list(reader)  # Зчитуємо всі рядки
            
            if len(rows) <= 1:
                print("У файлі немає студентів для видалення.")
                return None
            
            # Виводимо список студентів
            print("\nСписок студентів для видалення:")
            print(f"{rows[0][0]}  {rows[0][1]}  {rows[0][2]}")  # Заголовок
            print("-" * 40)
            
            for i in range(1, len(rows)):
                print(f"{i}. {rows[i][0]} | {rows[i][1]} | {rows[i][2]}")
                
            # Запитуємо номер студента для видалення
            student_num = int(input("\nВведіть номер студента для видалення: "))
            
            # Перевіряємо валідність номера (номери починаються з 1)
            if student_num < 1 or student_num >= len(rows):
                print(f"Помилка: Студента з номером {student_num} не існує.")
                return None
            
            # Отримуємо дані студента для підтвердження
            student_data = rows[student_num]
            print(f"\nВибраний студент для видалення:")
            print(f"Прізвище та ім'я: {student_data[0]}")
            print(f"Група: {student_data[1]}")
            print(f"Середній бал: {student_data[2]}")
            
            # Запитуємо підтвердження
            confirm = input("\nВи впевнені, що хочете видалити цього студента? (Enter - Так, n - Ні): ")
            if confirm != '':
                print("Видалення скасовано.")
                return None
            
            # Видаляємо студента (student_num вже відповідає індексу в rows)
            deleted_student = rows.pop(student_num)
            ...
            print(f"\nСтудент {deleted_student[0]} успішно видалений.")
            
    except ValueError:
        print("Помилка: Введіть правильний номер студента.")
    except IndexError:
        print("Помилка: Неправильний індекс студента.")
